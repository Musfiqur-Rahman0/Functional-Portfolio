...
const ProjectsPage = () => {
  ...
  const fetchCategoryData = async () => {
    try {
      setLoading(true);
      const res = await fetch("https://server-mpqtonacd-musfiqur-rahman0s-projects.vercel.app/categories");
  ...
  const fetchProductsByCategory = async (selectedCategory) => {
    ...
    try {
      const res = await fetch(
        `https://server-mpqtonacd-musfiqur-rahman0s-projects.vercel.app/projects?category=${selectedCategory}`, 
      );
      const data = await res.json();
